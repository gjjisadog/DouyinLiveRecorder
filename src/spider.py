...
async def get_douyin_app_stream_data(url: str, proxy_addr: OptionalStr = None, cookies: OptionalStr = None) -> dict:
    ...
    try:
        ...
        if len(web_rid) > 1:
            ...
            try:
                data = await get_sec_user_id(url, proxy_addr=proxy_addr, headers=headers)
                _room_id, _sec_uid = data
                room_data = await get_app_data(_room_id, _sec_uid)
            except UnsupportedUrlError:
                unique_id = await get_unique_id(url, proxy_addr=proxy_addr, headers=headers)
        ...
        LOGGER.warning("douyin_app_parse_failed error=%s", e)
...
async def get_douyin_web_stream_data(url: str, proxy_addr: OptionalStr = None, cookies: OptionalStr = None):
    ...
    try:
        ...
        LOGGER.warning("douyin_web_parse_failed error=%s", e)
        room_data = {'anchor_name': ""}
    return room_data
...
async def get_douyin_stream_data(url: str, proxy_addr: OptionalStr = None, cookies: OptionalStr = None) -> dict:
    ...
    try:
        ...
        LOGGER.warning("douyin_primary_parse_failed; switching resolver error=%s", e)
...
async def login_popkontv(
    ...
    try:
        ...
        async with httpx.AsyncClient(proxy=proxy_addr, timeout=20, verify=True) as client:
