...
- 第三阶段已加入可重复的 24/72 小时采样脚本，但当前分支没有跑满真实时长；
  多房间成功率、资源增长和站点风控变化仍需真实 NAS 时间验证。
- 共享 HTTP 客户端的代理配置取自 daemon 启动快照；Web 当前不管理代理。
  若人工热改 `proxy.url`，应重启容器以确保连接池切换到新代理。
