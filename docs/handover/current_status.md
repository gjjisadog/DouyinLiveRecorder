...
- 2026-04-03 已完成 `BL-011` 评估结论：
  - “真人手点打包版 EXE”不再作为发布前必做门槛
  - 当前文件桥自动化已覆盖核心验收链路
  - 若后续需要补真人点击，只作为可选观察项，不再阻塞 backlog 收口
- 2026-04-03 已完成 `BL-012` 第一轮增强：
  - 文件桥已新增任务维护命令：`edit_task`、`delete_task`、`import_tasks`、`export_tasks`
  - 已新增日志命令：`get_logs`、`clear_logs`
  - 已新增页面命令：`list_tabs`、`set_current_tab`
  - 相关回归已补进 `client/tests/test_automation_bridge.py`
