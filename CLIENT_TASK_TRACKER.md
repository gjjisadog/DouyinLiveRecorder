...
- 关键产物：
  - [acceptance_summary.json](./tmp_bl024_external_get_logs_run3\acceptance_summary.json)
  - [BL-024 session](./docs\sessions\2026-04-05-bl024-external-get-logs-ndtv.md)
- 验证命令：
...
- 已使用 Global News 外部公开 HLS 源完成一轮 `get_logs` 对照样本：
  - [acceptance_summary.json](./tmp_bl022_external_get_logs_run1\acceptance_summary.json)
  - [BL-022 session](./docs\sessions\2026-04-05-bl022-external-get-logs.md)
- 验证命令：
...
- 关键产物：
  - [acceptance_summary.json](./tmp_bl021_external_real_1gb_run1\acceptance_summary.json)
  - [BL-021 session](./docs\sessions\2026-04-05-bl021-external-real-1gb.md)
- 验证参数：
...
- 已修复带 query 的直链 HLS 会被识别成 `unknown` 的问题：
  - 修改 [platform_router.py](./client\core\platform_router.py)
  - 修改 [stream_resolver.py](./client\core\stream_resolver.py)
  - 新增 [test_platform_migration.py](./client\tests\test_platform_migration.py) 直链 query 用例
- 已补文件桥自动化的原子写入与重试：
  - 修改 [automation_bridge.py](./client\infra\process\automation_bridge.py)
  - 修改 [exe_automation_acceptance.py](./scripts\exe_automation_acceptance.py)
  - 扩展 [test_automation_bridge.py](./client\tests\test_automation_bridge.py)
- 已重打包客户端，使上述修正进入：
  - `dist/DouyinLiveRecorder Client/DouyinLiveRecorder Client.exe`
- `BL-023` 当前探索状态：
  - 已确认 `23 ABC / Uplynk` 可被新打包版按 `direct` 录制
  - 短样本产物：[acceptance_summary.json](./tmp_bl023_external_uplynk_probe2\acceptance_summary.json)
  - 长样本尝试产物：[acceptance_summary.json](./tmp_bl023_external_real_1gb_run2\acceptance_summary.json)
  - 当前未收口原因：
...
- 已更新 [CLIENT_RELEASE.md](./CLIENT_RELEASE.md)、[docs\handover\current_status.md](./docs\handover\current_status.md)、[docs\handover\known_issues.md](./docs\handover\known_issues.md)、[docs\handover\next_steps.md](./docs\handover\next_steps.md)、[docs\handover\executable_backlog.md](./docs\handover\executable_backlog.md)，把可发布结论与剩余边界集中沉淀。
- 本轮提交前最小验证已重跑：
...
- 为 [json_store.py](./client\infra\storage\json_store.py) 增加 JSON 读取容错和损坏文件隔离备份能力。
- 在 [config_service.py](./client\core\config_service.py)、[task_persistence.py](./client\core\task_persistence.py)、[history_service.py](./client\core\history_service.py) 接入容错回退逻辑。
- 历史记录加载改为“跳过坏记录、保留好记录”，避免单条脏数据拖垮整个历史页。
- 新增测试文件 [test_storage_resilience.py](./client\tests\test_storage_resilience.py)。
- 验证命令：
...
- 为 [enums.py](./client\core\enums.py)、[platform_router.py](./client\core\platform_router.py)、[stream_resolver.py](./client\core\stream_resolver.py) 补充第一批客户端已迁移平台。
- 本次接入的平台包括：网易 CC、千度热播、PandaTV、百度直播、ShowRoom、CHZZK。
- 同步更新了 [tasks_page.py](./client\ui\pages\tasks_page.py)、[history_page.py](./client\ui\pages\history_page.py)、[task_editor_dialog.py](./client\ui\dialogs\task_editor_dialog.py) 的平台显示文本。
- 新增测试文件 [test_platform_migration.py](./client\tests\test_platform_migration.py)，验证平台识别和解析分支接线。
- 验证命令：
...
- 扩展了 [settings_page.py](./client\ui\pages\settings_page.py) 的设置分组，新增覆盖录制调度、输出目录组织、后处理、代理平台范围等核心配置。
...
- 新增测试文件 [test_settings_page.py](./client\tests\test_settings_page.py)，验证设置页对扩展字段的加载和保存。
- 验证命令：
...
- 在 [bootstrap.py](./client\bootstrap.py) 接入启动前依赖检查流程，在主窗口创建前检查 `ffmpeg` 是否可用。
- 当检测不到 `ffmpeg` 时，客户端会弹出可理解提示，说明“仍可继续打开客户端，但暂时无法开始录制”，并允许用户选择继续或退出。
- 扩展了 [dependency_checker.py](./client\infra\env\dependency_checker.py)，让检查逻辑返回已检测到的 `ffmpeg` 路径，便于日志记录与测试。
- 新增测试文件 [test_bootstrap_dependencies.py](./client\tests\test_bootstrap_dependencies.py)，覆盖依赖满足、缺失后退出、缺失后继续三种分支。
- 验证命令：
...
- 新增测试文件 [test_core_services.py](./client\tests\test_core_services.py)，覆盖了 `Scheduler`、`RecordWorker`、`FfmpegService`、`NotificationService` 的核心分支。
...
- 在 [notification_service.py](./client\core\notification_service.py) 新增了 `Bark`、`ntfy`、`PushPlus`、`邮件` 四类远端通知渠道，并补充了对应的渠道别名、配置读取与结果汇总逻辑。
- 在 [settings_page.py](./client\ui\pages\settings_page.py) 的通知设置区域补充了新渠道所需字段和测试发送按钮。
- 邮件通知本次接入了 SMTP 服务器、账号、密码、发件邮箱、发件人名称、收件邮箱、端口和 SSL 开关等核心配置。
- 同步扩展了 [test_core_services.py](./client\tests\test_core_services.py) 与 [test_settings_page.py](./client\tests\test_settings_page.py)，验证新增渠道分发和设置页字段映射。
- 验证命令：
...
- 在 [settings_page.py](./client\ui\pages\settings_page.py) 新增“高级配置”区域，用键值编辑器方式接入 `Cookie`、`Authorization`、`账号密码` 三类原始配置。
...
- 扩展了 [test_settings_page.py](./client\tests\test_settings_page.py)，验证高级配置在设置页中的加载、解析与回写行为。
- 验证命令：
...
- 新增 [DOCKER_FLYINNAS.md](./DOCKER_FLYINNAS.md)，整理飞牛云 / NAS 场景下的目录挂载、启动命令和常见问题说明。
- 新增 [docker-compose.flyinnas.yaml](./docker-compose.flyinnas.yaml)，提供更适合 NAS 的 Compose 示例。
- 优化了 [Dockerfile](./Dockerfile) 与 [docker-compose.yaml](./docker-compose.yaml)，默认启用无交互模式，并使用 [requirements.docker.txt](./requirements.docker.txt) 避免在容器中安装桌面客户端依赖。
- 在 [main.py](./main.py) 增加了 `DLR_HEADLESS` 支持：当 `URL_config.ini` 为空且处于无交互环境时，程序会直接输出错误并退出，避免容器卡在 `input()`。
- 验证命令：
...
- 扩展 [.dockerignore](./.dockerignore)，排除 `.client-conda-env`、`dist`、`build`、日志目录和临时目录，避免镜像构建把本地开发产物一并打包进去。
- 新增 [healthcheck.py](./client\infra\docker\healthcheck.py) 与 [test_docker_healthcheck.py](./client\tests\test_docker_healthcheck.py)，把 Docker 健康检查固化为“`URL_config.ini` 非空 + `python main.py` 主进程仍在运行”。
- 更新 [Dockerfile](./Dockerfile)，加入 `HEALTHCHECK` 指令；同步重写 [docker-compose.yaml](./docker-compose.yaml) 与 [docker-compose.flyinnas.yaml](./docker-compose.flyinnas.yaml)，默认构建当前仓库版本，固定镜像标签为 `douyin-live-recorder:4.0.7`，并开启健康检查。
- 新增 [DOCKER_FLYINNAS_CHECKLIST.md](./DOCKER_FLYINNAS_CHECKLIST.md)，同时更新 [DOCKER_FLYINNAS.md](./DOCKER_FLYINNAS.md)，补齐飞牛云一键部署前置条件、启动命令、健康检查和回滚动作清单。
- 新增 [release.py](./client\infra\docker\release.py)、[test_docker_release.py](./client\tests\test_docker_release.py)、[build_docker_release.ps1](./build_docker_release.ps1)、[build_docker_release.bat](./build_docker_release.bat)，补齐 buildx 多架构发布脚本，并统一镜像版本标签来源到 [version.py](./client\version.py)。
- 新增 [.env.docker.example](./.env.docker.example)，把 Compose 默认镜像仓库名/标签提取为 `DLR_IMAGE_REPOSITORY` 与 `DLR_IMAGE_TAG`；同时更新 [build-image.yml](./.github\workflows\build-image.yml)，让 GitHub Actions 通过统一的 Docker 发布模块生成 tags/platforms。
- 新增 [DOCKER_FLYINNAS_REGRESSION.md](./DOCKER_FLYINNAS_REGRESSION.md)，把飞牛云回归清单按“首次部署 / 升级部署 / 回滚部署”三条路径拆开；同步更新 [README.md](./README.md) 的 Docker 使用说明，移除旧的 `latest` / 手改 Compose 指引。
- 验证命令：
...
& '.\.client-conda-env\python.exe' -m client.infra.docker.release print-tags --repository ghcr.io/gjjisadog/douyin-live-recorder
& '.\.client-conda-env\python.exe' -m client.infra.docker.release buildx --repository ghcr.io/gjjisadog/douyin-live-recorder --target daemon --repo-root '.' --push --dry-run
```
...
- 在 [task_persistence.py](./client\core\task_persistence.py) 增加任务文件导入、导出和合并能力，支持本地 JSON、旧版 `URL_config.ini` / `txt` 文本格式，以及按归一化 URL 去重合并。
- 新增“导入任务 / 导出任务”入口到 [tasks_page.py](./client\ui\pages\tasks_page.py)，导入后会同步刷新 `viewmodel`、补齐 `record_manager` 中的新任务，并落盘到本地 `tasks.json`。
- 导入合并策略为：同 URL 任务优先复用旧任务 ID，更新画质、显示名、启用状态；正在运行的任务不覆盖，计入跳过；导出时可回写旧版文本格式，便于回退或兼容主项目历史配置。
- 新增测试文件 [test_task_import_export.py](./client\tests\test_task_import_export.py)，覆盖旧版文本解析、JSON 导入、URL 去重合并、JSON/旧版文本导出，以及任务页导入后持久化链路。
- 验证命令：
...
- 在 [task_viewmodel.py](./client\viewmodels\task_viewmodel.py) 增加 `TaskActionResult`，把任务创建、编辑、导入、导出、删除、启停、默认画质、运行态判断、任务筛选和摘要统计统一沉到 `TaskViewModel`。
- 重构 [tasks_page.py](./client\ui\pages\tasks_page.py)，页面层改为只负责文件选择、编辑对话框、删除确认、表格渲染和结果提示；核心业务动作全部通过 `viewmodel` 执行。
- 新增测试文件 [test_task_viewmodel.py](./client\tests\test_task_viewmodel.py)，覆盖任务创建落盘、导入合并、删除持久化、启动失败错误回写，以及中文平台/状态筛选与摘要文案。
- 保留并复用已有的 [test_task_import_export.py](./client\tests\test_task_import_export.py)，确保任务页导入导出 UI 链路在重构后仍然可用。
- 验证命令：
...
- 新增 [CLIENT_STORAGE_STRATEGY.md](./CLIENT_STORAGE_STRATEGY.md)，明确客户端当前采用“JSON 主存储 + SQLite 迁移基座”的长期方案，并写清配置、任务、历史、迁移基座的边界与后续迁移顺序。
- 在 [storage_strategy.py](./client\infra\storage\storage_strategy.py) 实现存储策略清单服务，启动时会写出 `client_data/storage_meta.json`，记录模式、边界、迁移阶段和 SQLite 准备状态。
- 将 [sqlite_repo.py](./client\infra\storage\sqlite_repo.py) 从占位符升级为可用的 SQLite 基础仓储，支持初始化 `app_metadata` / `migrations` 表并读取 schema 版本与元信息。
- 在 [bootstrap.py](./client\bootstrap.py) 接入存储策略初始化；在 [app_settings.py](./client\app_settings.py) 增加 `storage_meta.json` 路径配置。
- 新增测试文件 [test_storage_strategy.py](./client\tests\test_storage_strategy.py)，覆盖存储策略清单写出、SQLite schema 元信息初始化和启动初始化链路。
- 验证命令：
...
- 新增 [version.py](./client\version.py) 作为客户端统一版本元数据入口，并在 [app_settings.py](./client\app_settings.py)、[main_window.py](./client\ui\main_window.py) 中接入产品名、版本号和运行时图标信息。
- 新增 [build_release.py](./client\build_release.py)、[build_client_release.ps1](./build_client_release.ps1)、[build_client_release.bat](./build_client_release.bat)、[requirements.client-build.txt](./requirements.client-build.txt)，补齐 Windows 客户端的 PyInstaller 打包入口、版本信息文件生成、资源目录打包和图标嵌入流程。
- 补充 [app_icon.svg](./client\resources\app_icon.svg)、[app_icon.ico](./client\resources\app_icon.ico) 与 [CLIENT_RELEASE.md](./CLIENT_RELEASE.md)，把发布依赖、构建命令、产物目录和人工检查项写成仓库内文档。
- 新增测试文件 [test_build_release.py](./client\tests\test_build_release.py)，覆盖默认路径、Windows 版本信息、PyInstaller 参数与图标装配逻辑。
- 为 [build_release.py](./client\build_release.py) 增加 conda `Library/bin` 运行时 DLL 收包逻辑，补齐 PySide6 / Shiboken / Qt 核心动态库，解决打包产物启动即崩问题。
...
```powershell
$exeDir = './dist\DouyinLiveRecorder Client'
...
- 在 [desktop_runtime.py](./client\infra\process\desktop_runtime.py) 新增桌面运行时服务，统一管理 `runtime_state.json`、Windows 开机启动注册表写入与异常退出恢复快照。
- 在 [app_settings.py](./client\app_settings.py)、[models.py](./client\core\models.py)、[config_service.py](./client\core\config_service.py) 补齐桌面常驻相关配置字段与本地序列化，包括最小化到托盘、关闭转托盘、开机启动、异常任务恢复、启动时恢复桌面状态。
- 在 [settings_page.py](./client\ui\pages\settings_page.py)、[main_window.py](./client\ui\main_window.py)、[bootstrap.py](./client\bootstrap.py) 接通托盘菜单、最小化/关闭转后台、启动快照恢复、异常任务恢复、退出时干净落盘以及启动链路中的运行时服务注入。
- 新增 [test_desktop_runtime.py](./client\tests\test_desktop_runtime.py)、[test_main_window_runtime.py](./client\tests\test_main_window_runtime.py)，并扩展 [test_settings_page.py](./client\tests\test_settings_page.py) 覆盖桌面配置加载/保存与开机启动状态回写。
- 验证命令：
...
- 扩展 [build_release.py](./client\build_release.py)，在 PyInstaller 目录版构建完成后自动生成版本化 zip、SHA256 校验清单、JSON 发布清单和 Markdown 发布清单，并补充 `--skip-package` 开关。
- 扩展 [test_build_release.py](./client\tests\test_build_release.py)，覆盖归档命名、zip 根目录结构、manifest 文件内容以及完整产物输出链路。
- 更新 [CLIENT_RELEASE.md](./CLIENT_RELEASE.md)，写明新增产物文件、PowerShell 校验命令、构建参数与发版检查步骤。
- 已在当前工作区实际生成以下发布文件：
...
- 新增 [flyinnas_regression.py](./client\infra\docker\flyinnas_regression.py)，把飞牛云 / NAS 的首次部署、升级部署、回滚部署回归整理成可执行脚本，覆盖挂载目录、URL 配置、Docker / Compose 可用性、Compose 解析、容器运行状态、健康检查、镜像标签和最近日志异常模式。
- 新增 [test_flyinnas_regression.py](./client\tests\test_flyinnas_regression.py)，覆盖首次部署通过、URL 配置为空失败、升级标签不匹配失败、日志异常失败等关键分支。
- 更新 [DOCKER_FLYINNAS.md](./DOCKER_FLYINNAS.md)、[DOCKER_FLYINNAS_CHECKLIST.md](./DOCKER_FLYINNAS_CHECKLIST.md)、[DOCKER_FLYINNAS_REGRESSION.md](./DOCKER_FLYINNAS_REGRESSION.md)，把飞牛云回归从纯手工清单补齐为“清单 + 命令”双轨说明。
- 飞牛云主机上的推荐执行方式：
...
- 新增 [flyinnas_deploy.py](./client\infra\docker\flyinnas_deploy.py)，实现本机到飞牛 NAS 的 SSH 部署链路：本地打包代码、通过 `scp` 上传、远端解包、保留已有配置、执行 `docker compose up -d --build`、再触发飞牛回归脚本。
- 新增 [deploy_flyinnas.ps1](./deploy_flyinnas.ps1) 与 [deploy_flyinnas.sh](./deploy_flyinnas.sh)，分别作为 Windows 和 Unix 侧的一键入口。
- 新增 [test_flyinnas_deploy.py](./client\tests\test_flyinnas_deploy.py)，覆盖部署包内容、`scp` / `ssh` 命令拼装、远端脚本关键行为和 `dry-run` 输出。
- 更新 [DOCKER_FLYINNAS.md](./DOCKER_FLYINNAS.md)，补充 SSH 一键部署的 PowerShell / Bash 用法、参数说明和行为边界。
- 验证命令：
...
- 原因：在 [bootstrap.py](./client\bootstrap.py) 中把提示回调的类型误写成了内建 `callable`，并与 `| None` 组合使用。
...
- 现象：在 [task_persistence.py](./client\core\task_persistence.py) 与 [tasks_page.py](./client\ui\pages\tasks_page.py) 上做局部 `apply_patch` 时，多次出现上下文匹配失败。
...
- 现象：将任务筛选逻辑从 [tasks_page.py](./client\ui\pages\tasks_page.py) 下沉到 [task_viewmodel.py](./client\viewmodels\task_viewmodel.py) 后，如果只按枚举值过滤，会丢失“抖音”“运行中”等中文关键词搜索能力。
...
- 现象：进入 `C-010` 后，仓库内已存在 [sqlite_repo.py](./client\infra\storage\sqlite_repo.py) 占位文件，存在“趁机把 config/tasks/history 一次性迁到 SQLite”的诱因。
...
- 处理：新增 [requirements.client-build.txt](./requirements.client-build.txt)，并通过 `& '.\.client-conda-env\python.exe' -m pip install -r requirements.client-build.txt` 为工程内解释器安装打包依赖。
- 结果：打包脚本可正常调用 `PyInstaller`，后续 dry-run 与真实构建均可执行。

#### 调试条目 010：Conda 环境下 PyInstaller 存在 Qt 依赖告警噪音

- 现象：真实构建成功后，控制台与 [warn-DouyinLiveRecorder Client.txt](./build\client-release\work\DouyinLiveRecorder Client\warn-DouyinLiveRecorder Client.txt) 中仍输出大量 `Qt6Core.dll`、`Qt6Gui.dll`、`libssl-3-x64.dll` 等未解析告警。
...
- 现象：从 [DouyinLiveRecorder Client.exe](./dist\DouyinLiveRecorder Client\DouyinLiveRecorder Client.exe) 冷启动后，没有进入主窗口，而是出现标题为 `Unhandled exception in script` 的异常窗口。
...
- 处理：在 [build_release.py](./client\build_release.py) 中新增 conda `Library/bin` DLL 收集逻辑，并补充 [test_build_release.py](./client\tests\test_build_release.py) 回归测试后重新打包。
...
- 处理：本轮改为通过 [healthcheck.py](./client\infra\docker\healthcheck.py) 单元测试、全量客户端测试、`py_compile` 和静态文件复核来收口 Docker 改动，并在文档中明确“尚缺镜像实构建验收”。
...
- 处理：调整 [release.py](./client\infra\docker\release.py) 为“`--push` 保持多架构，`--load` 自动退回到首个平台 `linux/amd64`”，并补充对应回归测试。
...
- 处理：将飞牛云回归收口为“可执行回归脚本 + 文档命令化 + 单元测试覆盖”，新增 [flyinnas_regression.py](./client\infra\docker\flyinnas_regression.py) 并配套更新三份飞牛云文档，让回归动作可以在 NAS 上直接执行。
...
- 现象：首次执行 [deploy_flyinnas.ps1](./deploy_flyinnas.ps1) 的 `-Host` 参数时，PowerShell 直接报 `Cannot overwrite variable Host because it is read-only or constant.`。
